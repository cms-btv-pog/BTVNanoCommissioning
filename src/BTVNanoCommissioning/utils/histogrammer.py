...
def histogrammer(events, workflow, year="2022", campaign="Summer22"):
    ...
    if "example" == workflow:
        ...
        _hist_dict["kindisc"] = Hist.Hist(syst_axis, flav_axis, bdt_axis, Hist.storage.Weight())

        _hist_dict["close_mlj"] = Hist.Hist(syst_axis, flav_axis, mass_axis, Hist.storage.Weight())
        _hist_dict["close_deta"] = Hist.Hist(syst_axis, flav_axis, eta_axis, Hist.storage.Weight())
        _hist_dict["close_dphi"] = Hist.Hist(syst_axis, flav_axis, phi_axis, Hist.storage.Weight())
        _hist_dict["close_ptrel"] = Hist.Hist(syst_axis, flav_axis, pt_axis, Hist.storage.Weight())
        _hist_dict["close_lj2ll_deta"] = Hist.Hist(syst_axis, flav_axis, eta_axis, Hist.storage.Weight())
        _hist_dict["close_lj2ll_dphi"] = Hist.Hist(syst_axis, flav_axis, phi_axis, Hist.storage.Weight())
        
        _hist_dict["far_mlj"] = Hist.Hist(syst_axis, flav_axis, mass_axis, Hist.storage.Weight())
        _hist_dict["far_deta"] = Hist.Hist(syst_axis, flav_axis, eta_axis, Hist.storage.Weight())
        _hist_dict["far_dphi"] = Hist.Hist(syst_axis, flav_axis, phi_axis, Hist.storage.Weight())
        _hist_dict["far_ptrel"] = Hist.Hist(syst_axis, flav_axis, pt_axis, Hist.storage.Weight())
        _hist_dict["far_lj2ll_deta"] = Hist.Hist(syst_axis, flav_axis, eta_axis, Hist.storage.Weight())
        _hist_dict["far_lj2ll_dphi"] = Hist.Hist(syst_axis, flav_axis, phi_axis, Hist.storage.Weight())

        _hist_dict["j2ll_deta"] = Hist.Hist(syst_axis, flav_axis, eta_axis, Hist.storage.Weight())
        _hist_dict["j2ll_dphi"] = Hist.Hist(syst_axis, flav_axis, phi_axis, Hist.storage.Weight())
    ...
    for syst in systematics:
        ...
        if "kindisc" in output.keys():
            output["kindisc"].fill(
                ...
                weight=flatten(ak.broadcast_arrays(weights.partial_weight(exclude=exclude_btv), pruned_ev.kindisc)[0]),
            )
            ...
            output["close_lj2ll_deta"].fill(
                ...
                weight=flatten(ak.broadcast_arrays(weight, pruned_ev.close_lj2ll_deta)[0]),
            )
            output["close_lj2ll_dphi"].fill(
                syst,
                flatten(pruned_ev.flavour),
                flatten(pruned_ev.close_lj2ll_dphi),
                weight=flatten(ak.broadcast_arrays(weight, pruned_ev.close_lj2ll_dphi)[0]),
            )
            output["far_mlj"].fill(
                syst,
                flatten(pruned_ev.flavour),
            ...
            output["far_lj2ll_deta"].fill(
                ...
                weight=flatten(ak.broadcast_arrays(weight, pruned_ev.far_lj2ll_deta)[0]),
            )
            output["far_lj2ll_dphi"].fill(
                syst,
                flatten(pruned_ev.flavour),
                flatten(pruned_ev.far_lj2ll_dphi),
                weight=flatten(ak.broadcast_arrays(weight, pruned_ev.far_lj2ll_dphi)[0]),
            )
