...
def run_das_command(cmd):
    ...
    if in_ci:
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.sh', delete=False) as script_file:
            ...
            script_file.write('    # Extract just the query portion\n')
            script_file.write('    QUERY=$(echo "' + escaped_cmd + '" | grep -o "\-query=[^ ]*" | sed \'s/-query=//\')\n')
            script_file.write('    # For file queries\n')
            script_file.write('    if [[ "' + escaped_cmd + '" == *"file dataset="* ]]; then\n')
            script_file.write('        # Fix file dataset query format\n')
            script_file.write('        DATASET=$(echo "' + escaped_cmd + '" | grep -o "dataset=[^ ]*" | sed \'s/dataset=//\')\n')
            script_file.write('        EXEC_CMD="$DASGOCLIENT_PATH -query=\\\"file dataset=$DATASET\\\"\"\n')
            script_file.write('    else\n')
            script_file.write('        # Use original query format\n')
