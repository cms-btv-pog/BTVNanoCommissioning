...
def getSF(flav, CvL, CvB, file="DeepCSV_ctagSF_MiniAOD94X_2017_pTincl.root",syst=""):    
    ...
    with importlib.resources.path(file[:file.rfind('/')].replace('/','.'), file[file.rfind('/')+1:]) as filename:
        f = uproot.open(filename)
    
    if syst == "" or syst == "central": systsuff = ""
    else: systsuff = '_'+syst
    
    SFd=np.array([f['SFl_hist'+systsuff].to_numpy()[0],f['SFc_hist'+systsuff].to_numpy()[0],f['SFb_hist'+systsuff].to_numpy()[0]])
    bins=[f['SFb_hist'+systsuff].to_numpy()[-1],f['SFb_hist'+systsuff].to_numpy()[-2],np.array([0,1,2,3])]
