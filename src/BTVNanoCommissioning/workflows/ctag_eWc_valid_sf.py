...
<<<<<<< HEAD
        ...
        for disc, axis in zip(disc_list, btag_axes):
            for i in range(1):
                _hist_sf_dict["%s_%d" %(disc,i)] = hist.Hist("Counts", dataset_axis, flav_axis,charge_axis,syst_axis,axis)
        for deepcsv, axises in zip(bininfo.keys(), btagDeepaxes):
             _hist_btagDeepdict["%s" %(deepcsv)] = hist.Hist("Counts", dataset_axis,flav_axis,charge_axis, axises)
...
=======
        ...
        _hist_btagDeepdict = {
            "pt": hist.Hist(
                "Counts", dataset_axis, flav_axis, charge_axis, jet_pt_axis
            ),
            "eta": hist.Hist(
                "Counts", dataset_axis, flav_axis, charge_axis, jet_eta_axis
            ),
            "phi": hist.Hist(
                "Counts", dataset_axis, flav_axis, charge_axis, jet_phi_axis
            ),
            "mass": hist.Hist(
                "Counts", dataset_axis, flav_axis, charge_axis, jet_mass_axis
            ),
        }
        for disc, axis in zip(disc_list, btag_axes):
            for i in range(1):
                _hist_sf_dict["%s_%d" % (disc, i)] = hist.Hist(
                    "Counts", dataset_axis, flav_axis, charge_axis, syst_axis, axis
                )
        for deepcsv, axises in zip(bininfo.keys(), btagDeepaxes):
            _hist_btagDeepdict["%s" % (deepcsv)] = hist.Hist(
                "Counts", dataset_axis, flav_axis, charge_axis, axises
            )
        ...
        _hist_event_dict = {
            ...
            "hl_eta": hist.Hist("Counts", dataset_axis, charge_axis, hard_leta_axis),
            "hl_phi": hist.Hist("Counts", dataset_axis, charge_axis, hard_lphi_axis),
            "hl_pfRelIso04_all": hist.Hist(
                "Counts", dataset_axis, charge_axis, hard_liso_axis
            ),
            "sl_pt": hist.Hist("Counts", dataset_axis, charge_axis, soft_lpt_axis),
            "sl_eta": hist.Hist("Counts", dataset_axis, charge_axis, soft_leta_axis),
            "sl_phi": hist.Hist("Counts", dataset_axis, charge_axis, soft_lphi_axis),
            "sl_pfRelIso04_all": hist.Hist(
                "Counts", dataset_axis, charge_axis, soft_liso_axis
            ),
            "sl_dxy": hist.Hist("Counts", dataset_axis, charge_axis, l_dxy_axis),
            "sl_dz": hist.Hist("Counts", dataset_axis, charge_axis, l_dz_axis),
            "sl_sip3d": hist.Hist("Counts", dataset_axis, charge_axis, l_sip3d_axis),
            "hlptratio": hist.Hist(
                "Counts", dataset_axis, flav_axis, charge_axis, hard_lpt_ratio_axis
            ),
            "slptratio": hist.Hist(
                "Counts", dataset_axis, flav_axis, charge_axis, soft_lpt_ratio_axis
            ),
            "zmass": hist.Hist("Counts", dataset_axis, charge_axis, zmass_axis),
            "zpt": hist.Hist("Counts", dataset_axis, charge_axis, zpt_axis),
            "zeta": hist.Hist("Counts", dataset_axis, charge_axis, zeta_axis),
            ...
            "metphi": hist.Hist("Counts", dataset_axis, metphi_axis),
            "drjet_smu": hist.Hist(
                "Counts", dataset_axis, flav_axis, charge_axis, dr_mujetsoftmu_axis
            ),
            "drjet_hmu": hist.Hist(
                "Counts", dataset_axis, flav_axis, charge_axis, dr_mujethardmu_axis
            ),
        }
>>>>>>> upstream/master
...
<<<<<<< HEAD
        ...
        if not isRealData:
            ...
            weights.add('lep1sf',np.where(event_level,eleSFs(ak.firsts(events.Electron[(events.Electron.pt > 34) & (abs(events.Electron.eta) < 2.5) & (events.Electron.mvaFall17V2Iso_WP80 > .5)]),self._campaign,correction_config[self._campaign]['LSF']),1.))
            weights.add('lep2sf',np.where(event_level,eleSFs(ak.firsts(events.Electron[(events.Electron.pt > 34) & (abs(events.Electron.eta) < 2.5) & (events.Electron.mvaFall17V2Iso_WP80 > .5)]),self._campaign,correction_config[self._campaign]['LSF']),1.))
        ...
        for histname, h in output.items():           
            if histname in self.btagDeephists:
                ...
                if(isRealData):
                    ...
                    h.fill(dataset=dataset,flav=smflav,char=osss, **fields,weight=weights.weight()[event_level]*osss)
                ...
                fields = {l: np.where(smuon_jet[histname.replace('_0','')]<0,-0.2,smuon_jet[histname.replace('_0','')]) for l in h.fields if l in dir(smuon_jet)}
                if isRealData:h.fill(dataset=dataset,flav=5,char=osss, syst='noSF',**fields,weight=osss)
                else :
                    h.fill(dataset=dataset,char=osss,flav=smflav, syst='noSF',**fields,weight=weights.weight()[event_level]*osss)
                    for syst in disc_list[histname.replace('_0','')][0].keys():
                        h.fill(dataset=dataset,flav=smflav,char=osss, syst=syst,**fields,weight=weights.weight()[event_level]*disc_list[histname.replace('_0','')][0][syst]*osss)
            
        
        if not isRealData:
            
            output['drmumu'].fill(dataset=dataset,char=osss,drmumu=ssmu.delta_r(shmu),weight=weights.weight()[event_level]*osss)
            output['hlptratio'].fill(dataset=dataset,flav=smflav,char=osss,hlptratio=shmu.pt/smuon_jet.pt,weight=weights.weight()[event_level]*osss)
            output['slptratio'].fill(dataset=dataset,flav=smflav,char=osss,slptratio=ssmu.pt/smuon_jet.pt,weight=weights.weight()[event_level]*osss)
            output['drjet_hmu'].fill(dataset=dataset,flav=smflav,char=osss,drjet_hmu=smuon_jet.delta_r(shmu),weight=weights.weight()[event_level]*osss)
            output['drjet_smu'].fill(dataset=dataset,flav=smflav,char=osss,drjet_smu=smuon_jet.delta_r(ssmu),weight=weights.weight()[event_level]*osss)
...
=======
        ...
        for histname, h in output.items():
            if histname in self.btagDeephists:
                ...
                fields = {l: shmu[l] for l in h.fields if l in dir(shmu)}
                if isRealData:
                    h.fill(dataset=dataset, char=osss, **fields, weight=osss)
                else:
                    h.fill(
                        dataset=dataset,
                        char=osss,
                ...
                if isRealData:
                    h.fill(dataset=dataset, **fields, weight=osss)
                else:
                    h.fill(
                        dataset=dataset,
                        **fields,
        ...
        if not isRealData:
            ...
            output["drmumu"].fill(
                dataset=dataset, char=osss, drmumu=ssmu.delta_r(shmu), weight=osss
            )
            output["hlptratio"].fill(
                dataset=dataset,
                flav=5,
