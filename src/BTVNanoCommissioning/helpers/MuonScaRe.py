# Adapted from https://gitlab.cern.ch/cms-nanoAOD/jsonpog-integration/-/blob/master/examples/MuonScaRe.py
...
class SeedSequence:
    ...
    def generate(self, n: int) -> List[int]:
        ...
        for k in range(n):
            ...
            buffer[k] = ((z * mix_const) & 0xFFFFFFFF) ^ (
                (buffer[k] << 13) & 0xFFFFFFFF
            )
...
def get_rndm(eta, phi, nL, evtNr, lumiNr, cset, nested=False):
    ...
    phi_int_f = (
        ((np.asarray(phi_f, dtype=np.float64) / math.pi) * (1 << 31 - 1)).astype(
            np.int64
        )
        & 0xFFF
    ).astype(np.uint32)
    ...
    seeds = [
        SeedSequence([np.uint32(ev), np.uint32(lumi), np.uint32(phi_int)]).generate(1)[
            0
        ]
    ...
    rndm_f = [get_rndm_f(seed) for seed in seeds]
...
def filter_boundaries(pt_corr, pt, nested, low_pt_threshold=26):
    ...
    if n_pt_outside > 0:
        print(
            f"There are {n_pt_outside} events with muon pt outside of ["
            + str(low_pt_threshold)
            + ",200] GeV. "
...
def pt_resol_var(pt_woresol, pt_wresol, eta, updn, cset, nested=False):
    ...
    pt_filter = (
        (pt_var_f / pt_woresol_f > 2) | (pt_var_f / pt_woresol_f < 0.1) | (pt_var_f < 0)
    )
