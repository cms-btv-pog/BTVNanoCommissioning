...
class NanoProcessor(processor.ProcessorABC):
    ...
    def process_shift(self, events, shift_name):
        ...
        # Selected objects #
        ####################
        event_level = event_level & req_softmu & req_mujet
        ...
        if not isRealData:
            ...
            smflav = smuj.hadronFlavour + 1 * (
                (smuj.partonFlavour == 0) & (smuj.hadronFlavour == 0)
            )
            ...
            smflav = ak.zeros_like(smuj.pt, dtype=int)

        if isRealData and len(selev) > 0:
            if self._year == "2022":
        ...
        for syst in systematics:
            ...
            output["soft_l_pfRelIso04_all"].fill(
                syst,
                flav=smflav[:, 0],
                pfRelIso03_all=flatten(ssmu[:, 0].pfRelIso04_all),
                weight=weight,
            )
            output["soft_l_dxy"].fill(
                syst,
                flav=smflav[:, 0],
                dxy=flatten(ssmu[:, 0].dxy),
                weight=weight,
            )
            output["soft_l_dz"].fill(
                syst,
                flav=smflav[:, 0],
                dz=flatten(ssmu[:, 0].dz),
                weight=weight,
            )
            output["soft_l_ptratio"].fill(
                syst,
                flav=smflav[:, 0],
                ratio=ssmu[:, 0].pt / smuj[:, 0].pt,
                weight=weight,
            )
            output["dr_lmujetsmu"].fill(
                syst,
                flav=smflav[:, 0],
                dr=smuj[:, 0].delta_r(ssmu[:, 0]),
                weight=weight,
            )
            output["soft_l_pt"].fill(
                syst,
                flav=smflav[:, 0],
                pt=flatten(ssmu[:, 0].pt),
                weight=weight,
            )
            output["soft_l_eta"].fill(
                syst,
                flav=smflav[:, 0],
                eta=flatten(ssmu[:, 0].eta),
                weight=weight,
            )
            output["soft_l_phi"].fill(
                syst,
                flav=smflav[:, 0],
                phi=flatten(ssmu[:, 0].phi),
                weight=weight,
            )
            output["soft_l_mass"].fill(
                syst,
                flav=smflav[:, 0],
                mass=flatten(ssmu[:, 0].mass),
                weight=weight,
            )

            output["njet"].fill(syst, njet, weight=weight)
            output["nssmu"].fill(syst, nssmu, weight=weight)
            output["nsmuj"].fill(syst, nsmuj, weight=weight)
            if "JetSVs" in events.fields:
                if len(lj_matched_JetSVs) > 0:
        ...
        #  Create root files  #
        #######################
        if self.isArray and len(selev) > 0:
