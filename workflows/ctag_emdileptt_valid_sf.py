...
class NanoProcessor(processor.ProcessorABC):
    ...
    def process(self, events):
        ...
        if selev.HLT.Mu23_TrkIsoVVL_Ele12_CaloIdL_TrackIdL_IsoVL:
            shmu =  selev.Muon[(selev.Muon.pt > 25) &(abs(selev.Muon.eta) < 2.4) & (selev.Muon.tightId > .5)&(selev.Muon.pfRelIso04_all<=0.15)]
            shele = selev.Electron[(selev.Electron.pt>15)&((abs(selev.Electron.eta) < 1.4442)|((abs(selev.Electron.eta) < 2.5)&(abs(selev.Electron.eta) >1.566)))& (selev.Electron.mvaFall17V2Iso_WP80 > .5)]
        else:
            shmu =  selev.Muon[(selev.Muon.pt > 14) &(abs(selev.Muon.eta) < 2.4) & (selev.Muon.tightId > .5)&(selev.Muon.pfRelIso04_all<=0.15)]
            shele = selev.Electron[(selev.Electron.pt>27)&((abs(selev.Electron.eta) < 1.4442)|((abs(selev.Electron.eta) < 2.5)&(abs(selev.Electron.eta) >1.566)))& (selev.Electron.mvaFall17V2Iso_WP80 > .5)]
        ...
        if not isRealData:
            if selev.HLT.Mu23_TrkIsoVVL_Ele12_CaloIdL_TrackIdL_IsoVL:
                weights.add('lep1sf',np.where(event_level,muSFs(ak.firsts(events.Muon[(events.Muon.pt>25)&(abs(events.Muon.eta) < 2.4)& (events.Muon.tightId > .5)&(events.Muon.pfRelIso04_all<=0.15)])),1.))
                weights.add('lep2sf',np.where(event_level,eleSFs(ak.firsts(events.Electron[(events.Electron.pt>15)&((abs(events.Electron.eta) < 1.4442)|((abs(events.Electron.eta) < 2.5)&(abs(events.Electron.eta) >1.566)))& (events.Electron.mvaFall17V2Iso_WP80 > .5)])),1.))
            else :
                weights.add('lep1sf',np.where(event_level,muSFs(ak.firsts(events.Muon[(events.Muon.pt>14)&(abs(events.Muon.eta) < 2.4)& (events.Muon.tightId > .5)&(events.Muon.pfRelIso04_all<=0.15)])),1.))
                weights.add('lep2sf',np.where(event_level,eleSFs(ak.firsts(events.Electron[(events.Electron.pt>27)&((abs(events.Electron.eta) < 1.4442)|((abs(events.Electron.eta) < 2.5)&(abs(events.Electron.eta) >1.566)))& (events.Electron.mvaFall17V2Iso_WP80 > .5)])),1.))
        isomu0 = shmu[:,0] 
        isomu1 = shele[:,0]
        # softmu0 = ssmu[:,0]
        # softmu1 = ssmu[:,1]
