    ...
    def process(self, events):
        ...
        if isRealData :
            ...
            if(all(i>1 for i in njet)):
                ...
                jetsfs_c_dn_sj = getSF(ak.to_numpy(sjets[:,1].hadronFlavour),ak.to_numpy(sjets[:,1].btagDeepCvL),ak.to_numpy(sjets[:,1].btagDeepCvB),deepjetc_sf,"TotalUncDown")
        ...
        if not isRealData:
            ...
            output['deepcsv_CvB_up_0'].fill(dataset=dataset,flav=smflav, deepcsv_CvB_up=smuon_jet.btagDeepCvL,weight=weights.weight()[event_level]*csvsfs_c_up_lj)
            ...
            output['deepcsv_CvB_up_0'].fill(dataset=dataset,flav=5, deepcsv_CvB_up=smuon_jet.btagDeepCvL)
