...
=======
        ...
        hard_leta_axis = hist.Bin("eta", r"Lepton $\eta$", 25, -2.5, 2.5)
        hard_lphi_axis = hist.Bin("phi", r"Lepton $\phi$", 30, -3, 3)
        hard_liso_axis = hist.Bin("pfRelIso04_all", r"Hard Muon Rel. Iso", 40, 0, 4.0)
...
<<<<<<< HEAD
        ...
        for histname, h in output.items():
            if histname in self.btagDeephists:
                ...
                if isRealData:h.fill(dataset=dataset,char=osss,flav=5, syst='noSF',**fields,weight=osss)
                else :
                    h.fill(dataset=dataset,flav=smflav,char=osss, syst='noSF',**fields,weight=weights.weight()[event_level]*osss)
                    for syst in disc_list[histname.replace('_0','')][0].keys():
                        h.fill(dataset=dataset,char=osss,flav=smflav, syst=syst,**fields,weight=weights.weight()[event_level]*disc_list[histname.replace('_0','')][0][syst]*osss)
