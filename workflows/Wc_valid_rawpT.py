...
class NanoProcessor(processor.ProcessorABC):
    ...
    def process(self, events):
        ...
        isomuon = events.Muon[(events.Muon.pt > 30) & (abs(events.Muon.eta) < 2.4) & (events.Muon.tightId > .5)&(events.Muon.pfRelIso04_all<0.15)] #
        
        # print(ak.all(events.Jet.metric_table(events.Muon) > 0.4, axis=2))
        # isomuon = ak.pad_none(isomuon, 1, axis=1) 
        # req_muon =(ak.count(isomuon.pt, axis=1) == 1)
        softmuon = events.Muon[(events.Muon.pt > 25) & (abs(events.Muon.eta) < 2.4) & (events.Muon.tightId > .5)&(events.Muon.pfRelIso04_all>0.5)] #
        ## Jet cuts 
        
        event_jet = events.Jet[((events.Jet.pt*(1-events.Jet.rawFactor))> 25) & (abs(events.Jet.eta) <= 2.4)&(events.Jet.puId > 0)&(events.Jet.jetId > 0)&(ak.all(events.Jet.metric_table(isomuon) > 0.4, axis=2))]
                                                                           
        muon_jet = event_jet
        req_jets = (ak.num(event_jet)>=1)
