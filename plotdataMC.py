...
for j in range(nj):
    for discr in arg.discr_list:
        ...
        scale_sf=1.
        if ('btag' in arg.discr_list or 'CvL' in arg.discr_list or 'CvB' in arg.discr_list) and arg.norm : scale_sf=sum(hflav_nosf[notdata].integrate("dataset").integrate("flav").integrate("ptwide").integrate("etawide").values()[()])/sum(hflav[notdata].integrate("dataset").integrate("flav").integrate("ptwide").integrate("etawide").values()[()])
        ...
        if 'btag' in arg.discr_list or 'CvL' in arg.discr_list or 'CvB' in arg.discr_list:
            ...
            if not arg.log:maximum=max(max((hflav[notdata].integrate("dataset").integrate("flav").integrate("ptwide").integrate("etawide").values()[()]+err_up)),max(data))
            ...
            ax=plot.plot1d(hflav[notdata].sum("dataset").sum("ptwide").sum("etawide"),overlay="flav",fill_opts={},error_opts=None,ax=ax,stack=True)
            ...
            plot.plot1d(hflav_nosf[notdata].sum("dataset").sum("ptwide").sum("etawide").sum("flav"),error_opts= {'linestyle': 'none','marker': 'o', 'markersize': 5.,'mfc': 'none','color' :'tab:pink' , 'elinewidth': 1.5},ax=ax,clear=False)
            ...
            plot.plotratio(
                                            ...
                                            clear=False)
            plot.plotratio(
                                            num=hflav_nosf[datas].sum("dataset").sum("flav").sum("etawide").sum("ptwide"),
                                            denom=hflav_nosf[notdata].sum("dataset").sum("flav").sum("etawide").sum("ptwide"),
                                            ax=rax,
            ...
            rax = plot.plotratio(
                                            ...
                                            clear=False)
        if(arg.log):
            if(arg.phase=="ctag"):ax.set_ylim(10,80000000)
        ...
        '''for i in range(len(ptbin)-1):    
            ...
            ax.legend(ncol=2,handles=ax.get_legend_handles_labels()[0],labels=['b','c','pileup','udsg','SFs Unc.','stat. Unc.','w/o SFs','%s'%(arg.ext)],fontsize=13)
            ax.set_xlabel(None)
            ax.set_xticklabels(ax.get_xticklabels(), fontsize=0)
            rax = plot.plotratio(
            ...
            if not arg.combine:name=str(j)
            print(name)
            if(arg.log):fig.savefig("plot/%s_unc_%s_pt%d_%d_%s_%s_%s.pdf" %(arg.phase, discrs, ptbin[i],ptbin[i+1],scale, arg.ext,name))
            else:fig.savefig("plot/%s_unc_lin_%s_pt%d_%d_%s_%s_%s.pdf" %(arg.phase, discrs, ptbin[i],ptbin[i+1],scale, arg.ext,name))
        ...
        for i in range(len(etabin)-1):   
            ...
            name="all"
            if not arg.combine:name=str(j)
            if(arg.log):fig.savefig("plot/%s_unc_%s_eta%.1f_%.1f_%s_%s_%s.pdf" %(arg.phase, discrs, etabin[i],etabin[i+1],scale, arg.ext,name))
            else:fig.savefig("plot/%s_unc_lin_%s_eta%.1f_%.1f_%s_%s_%s.pdf" %(arg.phase, discrs, etabin[i],etabin[i+1],scale, arg.ext,name))'''
