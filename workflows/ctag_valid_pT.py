...
deepcsvc_sf = BTagScaleFactor("data/DeepCSV_94XSF_V5_B_F.csv",BTagScaleFactor.RESHAPE,methods='iterativefit,iterativefit,iterativefit')
...
class NanoProcessor(processor.ProcessorABC):
    ...
    def __init__(self):        
        ...
        _hist_deepcsvSF_dict={
            'btagDeepBSF' : hist.Hist("Counts", dataset_axis, flav_axis, deepb_axis),
            'btagDeepCSF' : hist.Hist("Counts", dataset_axis, flav_axis, deepc_axis),
            'btagDeepFlavBSF' : hist.Hist("Counts", dataset_axis, flav_axis, deepjb_axis),
            'btagDeepFlavCSF' : hist.Hist("Counts", dataset_axis, flav_axis, deepjc_axis)
        }
    ...
    def process(self, events):
        ...
        if not isRealData:
            ...
            csvsfs_c=ak.fill_none(ak.prod(deepcsvc_sf.eval('central',sjets.hadronFlavour,abs(sjets.eta),sjets.pt,discr=sjets.btagDeepC),axis=-1),1.)
            ...
            output['btagDeepFlavBSF'].fill(dataset=dataset,flav=ak.flatten(genflavor), btagDeepFlavBSF=ak.flatten(sjets.btagDeepFlavB))
            output['btagDeepFlavCSF'].fill(dataset=dataset,flav=ak.flatten(genflavor), btagDeepFlavCSF=ak.flatten(sjets.btagDeepFlavC))
            output['btagDeepBSF'].fill(dataset=dataset,flav=ak.flatten(genflavor), btagDeepBSF=ak.flatten(sjets.btagDeepB))
            output['btagDeepCSF'].fill(dataset=dataset,flav=ak.flatten(genflavor), btagDeepCSF=ak.flatten(sjets.btagDeepC))
