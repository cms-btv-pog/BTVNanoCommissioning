...
for index, discr in enumerate(var_set):
    ...
    if args.sepflav:  # split into 3 panels for different flavor
        ...
        for i, c in enumerate(args.compared.split(",")):
            if "flav" in collated[c][discr].axes.name:
                callaxis = {"flav": sum}
            if "syst" in collated[c][discr].axes.name:
                callaxis["syst"] = "nominal"
            hep.histplot(
                collated[c][discr][callaxis],
                label=c,
                histtype=hist_type,
                yerr=True,
                ax=ax,
        ...
        # for c in args.compared.split(","):
        #     alls = collated[c][discr][callaxis] + alls
