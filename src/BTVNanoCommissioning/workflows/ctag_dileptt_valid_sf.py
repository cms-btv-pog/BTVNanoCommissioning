...
class NanoProcessor(processor.ProcessorABC):
    ...
    def process(self, events):
        ...
        event_level = req_trig & req_lumi & req_muon& req_dilepveto & req_dilepmass & req_MET & req_jets & req_softmu&req_mujet 
        ...
        sz=shmu[:,0]+shmu[:,1]
        if not isRealData:
            weights.add('lep1sf',np.where(event_level,muSFs(ak.firsts(events.Muon[(events.Muon.pt>12)&(abs(events.Muon.eta) < 2.4)& (events.Muon.tightId > .5)&(events.Muon.pfRelIso04_all<=0.15)]),self._campaign,correction_config[self._campaign]['LSF']),1.))
            weights.add('lep2sf',np.where(event_level,muSFs(ak.firsts(events.Muon[(events.Muon.pt < 25) & (abs(events.Muon.eta) < 2.4) & (events.Muon.tightId > .5)&(events.Muon.pfRelIso04_all>0.2)]),self._campaign,correction_config[self._campaign]['LSF']),1.))
        isomu0 = shmu[:,0] 
        isomu1 = shmu[:,1]
        ...
        if isRealData :
            ...
            if(all(i>1 for i in njet)):
                ...
                jetsfs_c[1]["SFdn"] = getSF(ak.to_numpy(sjets[:,1].hadronFlavour),ak.to_numpy(sjets[:,1].btagDeepCvL),ak.to_numpy(sjets[:,1].btagDeepCvB),self._deepjetc_sf,"TotalUncDown")
        ...
        for histname, h in output.items():
            ...
            if histname in self.btagDeephists:
                ...
                wei=ak.flatten(ak.broadcast_arrays(weights.weight()[event_level],sjets['pt'])[0])
                if(isRealData):
                    h.fill(dataset=dataset,flav=5, **fields)
                else:                    
                    h.fill(dataset=dataset,flav=ak.flatten(sjets.hadronFlavour), **fields,weight=wei)
