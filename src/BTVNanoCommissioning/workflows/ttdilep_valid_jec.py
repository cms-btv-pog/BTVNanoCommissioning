...
from BTVNanoCommissioning.utils.correction import lumiMasks,load_pu,load_BTV,load_jetfactory,add_jec_variables
...
class NanoProcessor(processor.ProcessorABC):
    ...
    def __init__(self,year="2017",campaign="Rereco17_94X"):  
        ...
        disc_list = ["btagCMVA", "btagCSVV2", 'btagDeepB', 'btagDeepC', 'btagDeepFlavB', 'btagDeepFlavC', 'btagDeepCvB', 'btagDeepCvL','btagDeepFlavCvB','btagDeepFlavCvL']
        ...
        for disc, axis in zip(disc_list, btag_axes):
            _hist_btagDeepdict[disc] = hist.Hist("Counts", dataset_axis, flav_axis, axis)
        ...
        ## Load corrections
        self._jet_factory = load_jetfactory(self._campaign,correction_config[self._campaign]['JME'])
    ...
    def process(self, events):
        ...
        if not isRealData: corrected_jets=self._jet_factory["mc"].build(add_jec_variables(events.Jet, events.fixedGridRhoFastjetAll),lazy_cache=events.caches[0])
        else :corrected_jets=self._jet_factory["data"].build(add_jec_variables(events.Jet, events.fixedGridRhoFastjetAll),lazy_cache=events.caches[0])
            ...
            if histname in self.btagDeephists:
                if(isRealData):
                    if histname == 'rawpt':
                        ...
                        fields = {l: np.where(sjets[l]<0,-0.2,sjets[l]) for l in h.fields if l in dir(sjets)}
                    ...
                    if histname == 'rawpt' :
                        ...
                        fields = {l: ak.flatten(np.where(sjets[l]<0,-0.2,sjets[l])) for l in h.fields if l in dir(sjets)}
