...
def run_das_command(cmd):
    ...
    if in_ci:
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.sh', delete=False) as script_file:
            ...
            script_file.write('    # Extract query type from command\n')
            script_file.write('    QUERY_TYPE=""\n')
            script_file.write('    if echo "' + escaped_cmd + '" | grep -q "file dataset="; then\n')
            script_file.write('        QUERY_TYPE="file"\n')
            script_file.write('    elif echo "' + escaped_cmd + '" | grep -q "site dataset="; then\n')
            script_file.write('        QUERY_TYPE="site"\n')
            script_file.write('    else\n')
            script_file.write('        QUERY_TYPE="dataset"\n')
            script_file.write('    fi\n')
            script_file.write('    \n')
            script_file.write('    # Process based on query type\n')
            script_file.write('    if [ "$QUERY_TYPE" = "file" ] || [ "$QUERY_TYPE" = "site" ]; then\n')
            script_file.write('        # Fix file/site dataset query format\n')
            script_file.write('        DATASET=$(echo "' + escaped_cmd + '" | grep -o "dataset=[^ ]*" | sed \'s/dataset=//\')\n')
            script_file.write('        EXEC_CMD="$DASGOCLIENT_PATH -query=\\"$QUERY_TYPE dataset=$DATASET\\""\n')
            script_file.write('    else\n')
            script_file.write('        # For dataset queries, use the full query string\n')
            script_file.write('        QUERY=$(echo "' + escaped_cmd + '" | grep -o "\\-query=[^ ]*" | sed \'s/-query=//\')\n')
            script_file.write('        EXEC_CMD="$DASGOCLIENT_PATH -query=\\"$QUERY\\""\n')
            script_file.write('    fi\n')
